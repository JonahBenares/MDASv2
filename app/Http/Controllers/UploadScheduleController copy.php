...
class UploadScheduleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $check_exist = PowerplantResource::pluck('resource_id')->all();
        $count=UploadScheduleTemp::where('resource_type','G')->where('upload_by',Auth::id())->whereNotIn('resource_name', $check_exist)->count();
        if($count==0){
            $data=UploadScheduleTemp::where('resource_type','G')->where('upload_by',Auth::id())->chunk(500, function($saveall) use(&$save) {
                $x=0;
                $data_insert=[];
                $id=[];
