...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('powerplants', function (Blueprint $table) {
            ...
            $table->foreignId('powerplant_type_id')->constrained(powerplant_type);
            $table->foreignId('subtype_id')->constrained(subtype);
            $table->string('operator')->nullable();
            $table->integer('participant_id')->default(0);
            $table->string('short_name')->nullable();
            $table->foreignId('region_id')->constrained(grid);
