    ...
    <div id="addType" tabindex="-1"  aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full bg-[#000000b8] ">
        <div class="relative w-full h-full max-w-lg md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow white:bg-gray-700">
                ...
                <form class="mx-20" method="POST" action="{{ route('powerplanttype.store') }}">
                    @csrf
                    <!-- Modal body -->
                    <div class="px-6">
    ...
    <div id="updateType" tabindex="-1"  aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full bg-[#000000b8] ">
        <div class="relative w-full h-full max-w-lg md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow white:bg-gray-700">
                ...
                <form action="{{ route('powerplanttype.update','1') }}" method='POST'>
                    @csrf
                    @method('PUT')
                    <!-- Modal body -->
                    <div class="px-6">
    ...
    <div id="addSub" tabindex="-1"  aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full bg-[#000000b8] ">
        <div class="relative w-full h-full max-w-lg md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow white:bg-gray-700">
                ...
                <form class="mx-20" method="POST" action="{{ route('insertSub') }}">
                    @csrf
                    <!-- Modal body -->
                    <div class="px-6">
                    ...
                    <!-- Modal footer -->
                    <div class="flex items-center p-6 space-x-2">
                        <input type="hidden" name='type_id' value="{{ (isset($_GET['id'])) ? $_GET['id'] : '' }}">
            ...
            <div class="w-10/12  ">
                <div class="flex justify-start space-x-2 overflow-x-auto">
                    ...
                    <div class="flex justify-center border border-gray-100 shadow-md rounded-lg mb-2">
                        <a href="{{route('show', $pt->id)}}" class="text-gray-900 bg-white font-medium rounded-lg rounded-r-none text-sm text-center inline-flex items-center white:focus:ring-gray-800 white:bg-white white:border-gray-700 white:text-gray-900 white:hover:bg-gray-200">
                            <span style="background-color:{{$pt->legend}}" class="w-5 rounded-r-none rounded-lg px-4 py-2 mr-2"><br></span> {{$pt->type_name}}
                        </a>
                        ...
                        <div id="dropdwon1" class="z-10 hidden font-normal bg-white  rounded-lg shadow w-auto white:bg-gray-700 white:divide-gray-600 mt-10">
                            <ul class="py-2 text-sm text-gray-700 white:text-gray-400" aria-labelledby="dropdownLargeButton">
                                  <li>
                                    <a data-modal-target="updateType" data-modal-toggle="updateType" data-id='{{$pt->id}}' data-type='{{$pt->type_name}}' data-color='{{$pt->legend}}' class="block px-4 py-2 hover:bg-gray-100 white:hover:bg-gray-600 white:hover:text-white editType">Update</a>
                                  </li>
                                <li>
