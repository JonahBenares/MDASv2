<x-app-layout>
    ...
    <div id="addType" tabindex="-1"  aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full bg-[#000000b8] ">
        <div class="relative w-full h-full max-w-lg md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow white:bg-gray-700">
                ...
                <form method="POST" action="{{ route('powerplanttype.store') }}">
                    @csrf
                    <!-- Modal body -->
                    <div class="px-6">
    ...
    <div id="addSub" tabindex="-1"  aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full bg-[#000000b8] ">
        <div class="relative w-full h-full max-w-lg md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow white:bg-gray-700">
                ...
                <form method="POST" action="{{ route('insertSub') }}">
                    @csrf
                    <!-- Modal body -->
                    <div class="px-6">
    ...
    <div class="p-4 ">
            ...
            <div class="w-10/12  ">
                <div class="flex justify-start space-x-2 overflow-x-auto">
                    ...
                    <div class="flex justify-center border border-gray-100 shadow-md rounded-lg mb-2 ">
                        ...
                        <button href="" data-dropdown-toggle="dropdwon{{$x}}" class="w-auto text-gray-900 bg-white  font-medium rounded-lg rounded-l-none text-sm px-1 py-2 text-center inline-flex items-center white:focus:ring-gray-800 white:bg-white white:border-gray-700 white:text-gray-900 white:hover:bg-gray-200">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 6.75a.75.75 0 110-1.5.75.75 0 010 1.5zM12 12.75a.75.75 0 110-1.5.75.75 0 010 1.5zM12 18.75a.75.75 0 110-1.5.75.75 0 010 1.5z" />
                            </svg>
                        </button>
                       
                        <div id="dropdwon{{$x}}" class="z-10 hidden font-normal bg-white  rounded-lg shadow w-auto white:bg-gray-700 white:divide-gray-600 mt-10">
                            <ul class="py-2 text-sm text-gray-700 white:text-gray-400" aria-labelledby="dropdownLargeButton">
                                ...
                                <li>
                                    <a href="{{ route('destroyType',['id'=>$pt->id]) }}" class="block px-4 py-2 hover:bg-gray-100 white:hover:bg-gray-600 white:hover:text-white" onclick="return confirm('Are you sure you want to delete this record?');">Delete</a>
                                </li>
                            </ul>
                        </div>
                    </div>
        ...
        <div class="relative overflow-y-auto h-80 rounded-lg mb-5 shadow-md border border-2" style="border-color:{{$legend}}">
            <div class="flex justify-start">
                <div class="w-20 py-1" style="background-color:{{$legend}}"><br></div>
                <div class="px-2 py-1">{{$powerplanttype}}</div>
            </div>
            @if(isset($_GET['id']))
                        ...
                        @foreach($subtype AS $s)
                            <tr class="bg-white border-b white:bg-gray-800 white:border-gray-700 hover:bg-gray-50 white:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-2 flex justify-center space-x-1">
                                    <a data-modal-target="updateSub" data-modal-toggle="updateSub" class="editSub" data-id='{{$s->id}}' data-type='{{$s->type_id}}' data-subtypename='{{$s->subtype_name}}'>
                                    ...
                                    <a href="{{ route('destroy',['id'=>$s->id,'type_id'=>$s->type_id]) }}" title="Delete" onclick="return confirm('Are you sure you want to delete this record?');">
