        ...
        <div class="relative w-full h-full max-w-lg md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow white:bg-gray-700 mt-20">
                ...
                <form method="POST" action="{{ route('insertNewOutage') }}">
                    ...
                    <input type="hidden" name="added_by" id='added_by' value="{{Auth::id()}}">
                    <!-- Modal footer -->
                ...
                @endphp
                    <tr class="{{ ($previousdate !== '' && $previousdate !== $date ) ? 'border-t-2 border-red-500' : '' }}">
                        <td class="border" align="center">
                            ...
                            <input type="hidden" name="resource_name[]" id='resource_name{{$x}}' value="{{ $sl->resource_name }}">
                            <input type="hidden" name="region_name[]" id='region_name{{$x}}' value="{{ $sl->region_name }}">
                            <input type="hidden" name="outage_date[]" id='outage_date{{$x}}' value="{{ $outage_date }}">
                            {{ $date }}
                        </td>
                        <td class="border" align="center">{{ getMaxOutage($sl->time_interval,$sl->resource_name,$sl->pp_type_id) }}</td>
                        <td class="border" align="center">{{ ($sl->pp_type_id!=0) ? getTypename($sl->pp_type_id) : '' }}</td>
                        <td class="border" align="center">{{ $sl->resource_name }}</td>
                        <td class="border" align="center">{{ number_format(getCapacity($sl->time_interval,$sl->resource_name,$sl->pp_type_id,$sl->resource_type),2) }}</td>
                        <td class="border" align="center">
                        ...
                        <td class="border" align="center">{{ ($previousdate !== $date ) ? number_format(getTotalCapacity($sl->time_interval,$sl->resource_type,$sl->region_name),2) : '' }}</td>
                    </tr>
